...
const STYLE_PROMPTS: Record<StyleKey, string> = {
  ...
  embroidery:
    "将图中的宠物转化为极度逼真的立体刺绣艺术风格（3D Embroidery / Patch Style）。【视觉核心】整个图案必须完全模拟真实的布艺刺绣效果：每一根「绣线」清晰可见，有真实的高光和阴影；图案整体微微隆起，呈现出从白色底布上凸起的3D立体感，边缘有真实刺绣特有的锁边线（Satin Stitch Edge）。【绣线质感】宠物毛发用平行密集的绣线表现，方向跟随毛发走向；不同色块之间有明确的绣线分界；整体色彩鲜艳饱和，接近刺绣常用的丝光绣线色。【边缘处理】图案外轮廓有一圈粗犷的锁边针迹（约3-5针宽度），通常为白色或金色，与主图案形成对比；整体像一枚可以直接缝在夹克上的「宠物刺绣徽章（Patch）」。【背景】纯白或极浅的棉布质感背景，让刺绣图案的立体感最大化。务必精准还原原图中宠物的品种特征、毛色与神态。",
  geometric:
